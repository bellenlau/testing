...
GitLab CI invokes this same launcher on a protected Leonardo login-node runner.
Manual use remains unchanged. CI uses `--run-id` to make its artifacts
discoverable and `--wait` to produce `summary.json` and `junit.xml` after Slurm
reaches a terminal state. The job fails when Slurm fails, accounting times out,
the status record is missing, or any QE category fails.
